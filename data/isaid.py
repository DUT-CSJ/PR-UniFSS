...
class DatasetISAID(DatasetPASCAL):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize) -> None:
        ...
        if self.split == 'trn':
            self.img_path = os.path.join(datapath, 'iSAID_5i/iSAID_patches/train/images')
            self.ann_path = os.path.join(datapath, 'iSAID_5i/iSAID_patches/train/semantic_png')
        else:
            self.img_path = os.path.join(datapath, 'iSAID_5i/iSAID_patches/val/images')
            self.ann_path = os.path.join(datapath, 'iSAID_5i/iSAID_patches/val/semantic_png')
        ...
        self.class_names = ['ship', 'storage tank', 'baseball diamond', 'tennis court', 'basketball court',
                            'ground track field', 'bridge', 'large vehicle', 'small vehicle', 'helicopter',
                            'swimming pool', 'roundabout', 'soccer ball field', 'plane', 'harbor']
        # self.class_names = ['ship', 'storage tank', 'baseball diamond', 'tennis court', 'basketball court',
        #                     'ground track field', 'bridge', 'large vehicle', 'small vehicle', 'helicopter',
        #                     'swimming pool', 'roundabout', 'football field', 'plane', 'harbor']
    ...
    def read_mask(self, img_name):
        ...
        mask = Image.open(os.path.join(self.ann_path, img_name) + '_instance_color_RGB.png')
        return mask
